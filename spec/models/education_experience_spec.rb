...
RSpec.describe EducationExperience, type: :model do
  ...
  it 'has a valid factory' do
    expect(FactoryGirl.create(:education_experience)).to be_valid
  end

  it 'is invalid without a school_name' do
    expect(FactoryGirl.build(:education_experience, school_name: nil)).not_to be_valid
  end

  it 'is invalid with an school_name less than 5 characters' do
    expect(FactoryGirl.build(:education_experience, school_name: 'aaa')).not_to be_valid
  end

  it 'is invalid with an school_name greater than 255 characters' do
    expect(FactoryGirl.build(:education_experience, school_name: Faker::Lorem.characters(256))).not_to be_valid
  end

  it 'is invalid without a diploma' do
    expect(FactoryGirl.build(:education_experience, diploma: nil)).not_to be_valid
  end

  it 'is invalid with a diploma less than 2 characters' do
    expect(FactoryGirl.build(:education_experience, diploma: 'a')).not_to be_valid
  end

  it 'is invalid with a diploma greater than 255 characters' do
    expect(FactoryGirl.build(:education_experience, diploma: Faker::Lorem.characters(256))).not_to be_valid
  end

  it 'is invalid without a start_date' do
    expect(FactoryGirl.build(:education_experience, start_date: nil)).not_to be_valid
  end

  describe '#date_range' do
    it "returns 'Month Year - present' without an end_date" do
      ed_exp = FactoryGirl.create(:education_experience,
